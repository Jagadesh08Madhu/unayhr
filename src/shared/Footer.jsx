  ...
  return (
    <section className="py-20 px-5 lg:px-20 font-lora bg-[#1F2937] text-white">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-14">
        ...
        >
          <h1 style={{letterSpacing:"5px"}} className="text-2xl font-bold mb-4">UnayHR PAYROLL</h1>
        ...
        >
          ...
          <ul className="space-y-2">
            {Items.map((item, index) => (
              ...
              key={index} className="hover:text-blue-800 text-lg transition-colors cursor-pointer">
                {item.name}
              </li>
            ))}
          </ul>
        ...
        >
          ...
          <p className='text-lg text-black'>For assistance or inquiries regarding payroll, feel free to call or message us anytime! 💼📞✨</p>
        </motion.div>
      </div>
    </section>
  );
